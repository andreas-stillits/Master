...
def _split_by_order(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for df in dfs:
        df.sort_values("resolution_factor", inplace=True)
    return dfs
...
def create_bc_adherence_figure(df: pd.DataFrame, ax0: plt.Axes, ax1: plt.Axes) -> None:
    ...
    for df, linestyle in zip([df1, df2], ["-", "--"], strict=True):
        ...
        ax0.plot(
            ...
            color=colors["curved_flux_direct"],
        )
        ax0.plot(
            x,
            fluc_top,
...
def create_convergence_figure(
    ...
    for df, linestyle in zip([df1, df2], ["-", "--"], strict=True):
        ...
        for key, label in zip(keys_flux, labels_flux, strict=True):
            q1s = _npy(df, key)
            q2 = true_flux if key != "top_flux_direct" else _npy(df2, key)[np.argmin(x)]
            if (
                key == "top_flux_direct"
...
def plot_validation_results(
    ...
    label_panel(ax3, "(e)")
    save(fig, output_dir / "validation.pdf")
    #
    if show:
        plt.show()
    plt.close("all")
